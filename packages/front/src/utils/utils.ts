export const filesToDataURL = async (files : any[]) => {
...
export const dataURLToFile = async (files: any[]) => {
  return new Promise((resolve) => {
    Promise.all(
...
export const encodeFileToText = async (file : any) => {
  return file.text().then((text: any) => {
    return text;
  });
}

export const getUniqFiles = async (newFiles: any, files: any[]) => {
  return new Promise((resolve) => {
...
export const arrayFilesToFileList = (filesList : any) => {
